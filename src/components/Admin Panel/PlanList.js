...
const PlansList = () => {
  ...
  const handleAddPlan = async (formData) => {
    ...
    try {
      const response = await fetch('http://localhost:8001/api/plan_list', {
        method: 'POST',
        headers: {
  ...
  const handleUpdatePlan = async (formData) => {
    try {
      await axios.put(`http://localhost:8001/api/plan_list/${currentPlan.pid}`, formData);
  ...
  const handleDeleteClick = async (planToDelete) => {
    if (window.confirm("Are you sure you want to delete this plan?")) {
      try {
        await axios.delete(`http://localhost:8001/api/plan_list/${planToDelete.pid}`);
...
const AddPlanForm = ({
  ...
  const handleAddCategory = async () => {
    ...
    if (trimmedCategory && !categories.map(cat => cat.add_category).includes(trimmedCategory)) {
      try {
        const response = await fetch('http://localhost:8001/api/add_category', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
